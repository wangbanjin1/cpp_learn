...
auto output_element(std::ostream& os, const T& element, const Cont&,
                    std::true_type) -> decltype(std::declval<typename Cont::key_type>(), os);
...
template <typename T, typename = std::enable_if_t<!has_output_function_v<std::remove_cv_t<std::remove_reference_t<T>>>>>
auto operator<<(std::ostream& os, T&& container) -> decltype(output_range::adl_begin(std::forward<T>(container)),
                                                             output_range::adl_end(std::forward<T>(container)), os) {
    using std::decay_t;
    using std::is_same_v;

    using element_type = decay_t<decltype(*output_range::adl_begin(std::forward<T>(container)))>;
    ...
    for (auto it = output_range::adl_begin(std::forward<T>(container)); it != end; ++it) {
        if constexpr (is_char_v) {
            if (*it == '\0') {
                break;
            }
        ...
        output_element(os, *it, std::forward<T>(container), is_pair<element_type>());
    }
    if constexpr (!is_char_v) {
...
std::ostream& operator<<(std::ostream& os, const std::tuple<Args...>& args) {
    os << '(';
    output_tuple_members(os, args, std::make_index_sequence<std::tuple_size_v<std::tuple<Args...>>>{});
    os << ')';
    return os;
}
